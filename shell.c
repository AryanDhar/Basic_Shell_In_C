...
char** parseInput(char *input)
{
    ...
    char **tokens = malloc(buf_size*sizeof(char*)); // heap memory allocated to persist memory between function calls.
    char *tok;
    while((tok = strsep(&input," ")) != NULL) 
    {
        if(strlen(tok) != 0) // to avoid more than one space char... 
        {
            tokens[pos++] = tok;
        }
    }
...
void executeParallelCommands(char **tokens)
{
    ...
    while(tokens[i])
    {
        ...
        process_count++;
        tokens[i] = NULL; // so that execvp  doesn;t execute further commands...
...
int main()
{
	...
	while(1)	// This loop will keep your shell running until user exits.
	{
        ...
        int c = 0;
        for(int i=0; args[i]!= NULL ; i++)  // Either complete sequential or parralel execution...
        {
